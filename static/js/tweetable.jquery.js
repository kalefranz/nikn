...
(function($) {
	...
	jQuery.fn.tweetable = function (opts) {
		...
		return this.each(function () {
			...
			var act = jQuery(this);
			var tweetList = jQuery('<ul class="tweetList list-group">')[opts.position.toLowerCase() + 'To'](act);
			...
			{
				tweetList.append('<li class="list-group-item tweet_content_' + i + '"><p class="tweet_link_' + i + '">' + tweet.response.replace(/#(.*?)(\s|$)/g, '<span class="hash">#$1 </span>').replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, '<a href="$&">$&</a> ').replace(/@(.*?)(\s|\(|\)|$)/g, '<a href="http://twitter.com/$1">@$1 </a>$2').replace(/:">/, ' ">').replace(/: <\/a>/, '</a>:') + '</p></li>');
			};
	...
	$.fn.tweetable.options = {
		...
		username: 'kalefranz',         // @username tweets to display
